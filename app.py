...
with st.sidebar:
    ...
    fund_top_n = st.slider("Top N por I.S.", 5, 50, 25, 5,
                            help="Busca dados fundamentalistas dos N mais sobrevendidos")
...
if st.session_state.get('run') and 'oportunidades' not in st.session_state:
    ...
    # Busca fundamentalista dos top N por IS
    top_tickers = [o['Ticker'] for o in sorted(oportunidades, key=lambda x: x['IS'], reverse=True)[:fund_top_n]]
    fund_cache = {}
    pb_f = st.progress(0, text=f"💼 Análise fundamentalista (top {fund_top_n})…")
    for i, tk in enumerate(top_tickers):
        fund_cache[tk] = buscar_fundamentalista(tk)
        pb_f.progress((i + 1) / len(top_tickers))
    pb_f.empty()
