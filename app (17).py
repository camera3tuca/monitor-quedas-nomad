...
<style>
    ...
    .main-title  { color:white; font-size:1.9rem; font-weight:700; margin:0; }
    .main-subtitle { color:rgba(255,255,255,0.85); font-size:0.92rem; margin:0.4rem 0 0 0; }
    .section-header {
        color:#1a237e; font-size:1.3rem; font-weight:600;
        margin:1.5rem 0 0.75rem 0; padding-bottom:0.4rem;
        border-bottom:2px solid #1a237e;
    }
    .tip-box { background:linear-gradient(135deg,#f5f7fa,#c3cfe2); padding:0.75rem 1rem; border-radius:8px; margin-bottom:1rem; }
    .tip-box p { margin:0; color:#334155; font-size:0.9rem; }
    .select-prompt { background:linear-gradient(135deg,#e0e7ff,#c7d2fe); padding:1.5rem 2rem; border-radius:8px; text-align:center; margin:1rem 0; }
    ...
    .stButton > button {
        ...
        padding:0.6rem 1.5rem !important; border-radius:8px !important; width:100% !important;
    }
    div[data-testid="metric-container"] {
        background:#f8fafc; border:1px solid #e2e8f0; padding:0.8rem 1rem; border-radius:8px;
    }
...
# ─────────────────────────────────────────────────────────────────────────────
# LISTA COMPLETA DE AÇÕES E ETFs NOMAD (~992 tickers)
# ─────────────────────────────────────────────────────────────────────────────
NOMAD_STOCKS = {
...
def nome_curto(ticker, maxlen=24):
...
def calcular_indicadores(df, pb=None):
    df_calc = df.copy()
...
def analisar_oportunidades(df_calc, pb=None):
    resultados = []
    tickers = df_calc.columns.get_level_values(1).unique()
    for i, ticker in enumerate(tickers):
        try:
...
def buscar_fundamentalista(ticker):
    try:
        ...
        if pe and isinstance(pe, (int, float)):
            det['P/E'] = round(pe, 2)
            if 10 <= pe <= 25:   score += 15; det['P/E_pts'] = '+15'; det['P/E_av'] = 'Ótimo (10-25)'
            elif pe <= 35:       score += 10; det['P/E_pts'] = '+10'; det['P/E_av'] = 'Bom (25-35)'
            elif pe > 50:        score -= 10; det['P/E_pts'] = '-10'; det['P/E_av'] = 'Muito alto (>50)'
            else:                             det['P/E_pts'] = '+0';  det['P/E_av'] = 'Regular'
        ...
        return {'score': score, 'classe': classe, 'label': label, 'grad': grad, 'tc': tc,
                'det': det, 'setor': info.get('sector', ''), 'rec_raw': rec,
                'pe': pe, 'dy': dy, 'rg': rg, 'mc': mc}
    except Exception:
        return None


def enriquecer(oportunidades, top_n=25, pb=None):
    top_tickers = [o['Ticker'] for o in sorted(oportunidades, key=lambda x: x['IS'], reverse=True)[:top_n]]
    cache = {}
    for i, tk in enumerate(top_tickers):
        cache[tk] = buscar_fundamentalista(tk)
        if pb: pb.progress((i + 1) / len(top_tickers))
    for o in oportunidades:
        fd = cache.get(o['Ticker'])
        if fd:
            o['Score_Fund']  = fd['score']
            o['Classe_Fund'] = fd['classe']
            o['Score_Total'] = round(min(100, o['Score'] * 15) * 0.6 + fd['score'] * 0.4, 1)
        else:
            o['Score_Total'] = round(min(100, o['Score'] * 15) * 0.6, 1)
    return oportunidades, cache


# ─────────────────────────────────────────────────────────────────────────────
# ESTILOS DA TABELA (pandas Styler)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
def plotar_grafico(df_t, ticker):
    close  = df_t['Close']
    ...
    fig = make_subplots(rows=3, cols=1, row_heights=[0.6, 0.2, 0.2], shared_xaxes=True,
                        ...
                        subplot_titles=[f"📈 {ticker} — {nome_curto(ticker)}", "RSI 14", "Estocástico %K"])
    if 'Open' in df_t.columns and 'High' in df_t.columns:
        fig.add_trace(go.Candlestick(
            x=df_t.index, open=df_t['Open'], high=df_t['High'], low=df_t['Low'], close=close,
    ...
    if bb_low is not None and bb_up is not None:
        ...
        fig.add_trace(go.Scatter(x=bb_low.index, y=bb_low,
                                 ...
                                 showlegend=False), row=1, col=1)
    if ema20  is not None: fig.add_trace(go.Scatter(x=ema20.index,  y=ema20,  name='EMA20',  line=dict(color='#42a5f5', width=1.2)), row=1, col=1)
    if ema50  is not None: fig.add_trace(go.Scatter(x=ema50.index,  y=ema50,  name='EMA50',  line=dict(color='#ff7043', width=1.2)), row=1, col=1)
    if ema200 is not None: fig.add_trace(go.Scatter(x=ema200.index, y=ema200, name='EMA200', line=dict(color='#66bb6a', width=1.8)), row=1, col=1)
    for nivel, pf in fibs.items():
        fig.add_hline(y=pf, line_dash="dot", line_color=fib_cores[nivel], line_width=0.8,
                      annotation_text=f" {nivel}", annotation_position="right",
                      annotation_font_color=fib_cores[nivel], annotation_font_size=9, row=1, col=1)
    if rsi_s is not None:
        fig.add_trace(go.Scatter(x=rsi_s.index, y=rsi_s, name='RSI14',
                                 line=dict(color='#ff8f00', width=1.4)), row=2, col=1)
    ...
    fig.update_layout(height=600, template='plotly_dark', paper_bgcolor='#0e1117',
                      plot_bgcolor='#0e1117', legend=dict(orientation='h', y=1.06, x=0),
                      margin=dict(l=40, r=60, t=60, b=20), xaxis_rangeslider_visible=False)
...
with st.sidebar:
    ...
    setores_sel = st.multiselect("Setores (vazio = todos)", options=setores_todos, default=[],
                                  placeholder="Todos os setores")
    tickers_analise = ([tk for tk, (_, s) in NOMAD_STOCKS.items() if s in setores_sel]
                       if setores_sel else list(NOMAD_STOCKS.keys()))
    st.info(f"🗂️ **{len(tickers_analise)} tickers** na varredura")

    st.markdown("---")
    st.markdown("### 🎯 Filtros de Resultado")
    col_s1, col_s2 = st.columns(2)
    with col_s1:
        f_is_min  = st.slider("I.S. mínimo",  0, 100, 0, 5,
                               help="0 = todos | 75+ = muito sobrevendido")
    with col_s2:
        f_liq_min = st.slider("Liquidez mín.", 0,  10, 0, 1)
    f_potencial = st.selectbox("Potencial mínimo", ['Todos', 'Média', 'Alta', 'Muito Alta'])

    st.markdown("### 📈 EMAs (tendência de alta)")
    st.markdown("""
    <div class="tip-box">
        <p>💡 Selecione as médias móveis para filtrar ações em correção dentro de tendências de alta</p>
    </div>""", unsafe_allow_html=True)
    colf1, colf2 = st.columns(2)
    with colf1:
        f_ema20  = st.checkbox("Acima da EMA20 (curto prazo)",  value=False)
        f_ema50  = st.checkbox("Acima da EMA50 (médio prazo)",  value=False)
    with colf2:
        f_ema200 = st.checkbox("Acima da EMA200 (longo prazo)", value=False)

    st.markdown("---")
    st.markdown("### 💼 Fundamentalista")
    fund_top_n = st.slider("Top N análise fundamentalista", 5, 50, 25, 5)

    st.markdown("---")
    if st.button("🔍 Executar Varredura", type="primary"):
        st.session_state['run'] = True
        st.session_state.pop('oportunidades', None)
        st.session_state.pop('df_calc', None)
    ...
    st.markdown("""
    <div style="text-align:center;font-size:0.72rem;color:#888;margin-top:1rem;">
...
<div class="main-header">
    ...
    <p class="main-subtitle">
        {dia_pt}, {agora.strftime('%d/%m/%Y às %H:%M')} (Brasília) &nbsp;|&nbsp;
...
with st.expander("📚 Guia dos Indicadores — Entenda os Sinais", expanded=False):
    ...
    Combina RSI e Estocástico — **75-100**: 🔴 Muito sobrevendido | **60-75**: 🟠 Moderado | **<60**: ⚪ Neutro

    ### 📊 EMAs — Como usar os filtros
    Ativo em **queda** mas **acima das EMAs** = correção em tendência de alta = **oportunidade**!

    ### 🌟 Fibonacci 61.8% — Zona de Ouro
    O nível mais importante para reversão. Preço próximo de 61.8% = alta probabilidade de suporte.

    ### 💡 Estratégia resumida
    1. Filtre por **EMA200** para garantir tendência de alta
    2. Procure **I.S. > 75** (forte sobrevenda)
    3. Confirme com **RSI < 30** e **Stoch < 20**
    4. Verifique se está próximo do **Fibonacci 61.8%**
    5. Clique na linha para ver o **gráfico completo** 📈
    """)
...
if st.session_state.get('run') and 'oportunidades' not in st.session_state:
    with st.status("⬇️ Baixando dados de mercado...", expanded=True) as status:
        pb1 = st.progress(0.0)
        df = buscar_dados(tuple(tickers_analise))
        pb1.progress(1.0)
        if df.empty:
            status.update(label="❌ Falha no download", state="error")
            st.error("Não foi possível baixar dados. Verifique a conexão.")
            st.stop()
        n_ok = len(df.columns.get_level_values(1).unique())
        st.success(f"✅ {df.shape[0]} dias históricos | {n_ok} tickers válidos")
        status.update(state="complete", expanded=False)

    with st.status("🔢 Calculando indicadores técnicos...", expanded=True) as status:
        pb2 = st.progress(0.0)
        df_calc = calcular_indicadores(df, pb2)
        st.success("✅ Indicadores prontos")
        status.update(state="complete", expanded=False)

    with st.status("🔍 Varrendo oportunidades...", expanded=True) as status:
        pb3 = st.progress(0.0)
        oportunidades = analisar_oportunidades(df_calc, pb3)
        st.success(f"✅ {len(oportunidades)} oportunidades detectadas")
        status.update(state="complete", expanded=False)

    with st.status(f"💼 Análise fundamentalista (top {fund_top_n})...", expanded=True) as status:
        pb4 = st.progress(0.0)
        oportunidades, fund_cache = enriquecer(oportunidades, fund_top_n, pb4)
        st.success("✅ Fundamentalista concluído")
        status.update(state="complete", expanded=False)
...
if 'oportunidades' not in st.session_state:
    ...
    <div style="text-align:center;padding:3rem;color:#888;">
        ...
        <p>Configure os filtros na barra lateral e clique em <b>🔍 Executar Varredura</b></p>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
c1.metric("📋 Tickers válidos", n_ok)
c2.metric("🔍 Oportunidades",   len(oportunidades))
c3.metric("🔥 Maior I.S.",      f"{df_opp['IS'].max():.1f}" if not df_opp.empty else "—",
          delta=df_opp.loc[df_opp['IS'].idxmax(), 'Ticker'] if not df_opp.empty else "")
c4.metric("📉 Maior Queda",     f"{df_opp['Queda_Dia'].min():.2f}%" if not df_opp.empty else "—",
          delta=df_opp.loc[df_opp['Queda_Dia'].idxmin(), 'Ticker'] if not df_opp.empty else "")
c5.metric("🌟 Potencial",
          f"{len(df_opp[df_opp['Potencial']=='Muito Alta'])} Muito Alta" if not df_opp.empty else "0")
...
if f_potencial != 'Todos':
    df_res = df_res[df_res['Potencial'].map(lambda x: ordens.get(x, -1)) >= ordens[f_potencial]]
if f_is_min > 0:
    df_res = df_res[df_res['IS'] >= f_is_min]
if f_liq_min > 0:
    df_res = df_res[df_res['Liquidez'] >= f_liq_min]

if (f_ema20 or f_ema50 or f_ema200) and df_calc is not None:
    mask_ema = []
    for _, row in df_res.iterrows():
        ...
        try:
            ...
            ok = True
            if f_ema20  and preco <= df_t['EMA20'].iloc[-1]:  ok = False
            if f_ema50  and ok and preco <= df_t['EMA50'].iloc[-1]:  ok = False
            if f_ema200 and ok and preco <= df_t['EMA200'].iloc[-1]: ok = False
            mask_ema.append(ok)
        except Exception:
            mask_ema.append(False)
    df_res = df_res[mask_ema].reset_index(drop=True)

if len(df_res) < len(df_opp):
    filtros_str = []
    if f_ema20:  filtros_str.append("EMA20")
    if f_ema50:  filtros_str.append("EMA50")
    if f_ema200: filtros_str.append("EMA200")
    if f_is_min > 0:     filtros_str.append(f"I.S.≥{f_is_min}")
    if f_liq_min > 0:    filtros_str.append(f"Liq.≥{f_liq_min}")
    if f_potencial != 'Todos': filtros_str.append(f"Pot.≥{f_potencial}")
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#d4fc79,#96e6a1);padding:0.75rem 1rem;border-radius:8px;margin-bottom:0.5rem;">
        <p style="margin:0;color:#166534;font-weight:600;">
            ✅ <b>{len(df_res)}</b> ações encontradas &nbsp;|&nbsp; Filtros: {' + '.join(filtros_str)}
            &nbsp;|&nbsp; <span style="font-weight:400;">{len(df_opp)-len(df_res)} descartadas</span>
        </p>
    </div>""", unsafe_allow_html=True)

if df_res.empty:
    st.markdown("""
    <div style="background:linear-gradient(135deg,#ffeaa7,#fdcb6e);padding:1.5rem;border-radius:8px;text-align:center;">
        <h3 style="margin:0;color:#7c3626;">⚠️ Nenhuma ação passou pelos filtros</h3>
        <p style="margin:0.5rem 0 0;color:#7c3626;">Relaxe um ou mais critérios e tente novamente.</p>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
with col_t1:
    ...
    top_is = df_res.nlargest(5, 'IS')[['Ticker', 'Empresa', 'IS', 'Queda_Dia', 'Potencial']].copy()
    top_is['IS']        = top_is['IS'].apply(lambda x: f"{x:.1f}")
    top_is['Queda_Dia'] = top_is['Queda_Dia'].apply(lambda x: f"{x:.2f}%")
    st.dataframe(top_is, hide_index=True, use_container_width=True)
with col_t2:
    st.markdown("#### 📉 Top 5 — Maiores Quedas do Dia")
    top_q = df_res.nsmallest(5, 'Queda_Dia')[['Ticker', 'Empresa', 'Queda_Dia', 'IS', 'Potencial']].copy()
    top_q['Queda_Dia'] = top_q['Queda_Dia'].apply(lambda x: f"{x:.2f}%")
    top_q['IS']        = top_q['IS'].apply(lambda x: f"{x:.1f}")
...
st.markdown("""
<div class="tip-box">
    <p>💡 <strong>Dica:</strong> Clique em qualquer linha da tabela para visualizar o gráfico técnico completo e a análise fundamentalista</p>
</div>""", unsafe_allow_html=True)
...
evento = st.dataframe(
    styled,
    column_order=("Ticker", "Empresa", "Liquidez", "Preco", "Queda_Dia", "IS",
                  "Volume", "Gap", "Potencial", "Score", "Sinais"),
    column_config={
        "Empresa":  st.column_config.TextColumn("Empresa",     width="medium"),
        "Liquidez": st.column_config.NumberColumn("💧 Liq.",   width="small",
                        help="Ranking de liquidez 0-10 (🔴 baixa → 🟢 alta)"),
        "IS":       st.column_config.NumberColumn("I.S.",      help="Índice de Sobrevenda (RSI+Stoch)"),
...
st.download_button("⬇️ Exportar CSV", data=csv,
                   file_name=f"nomad_{agora.strftime('%Y%m%d_%H%M')}.csv", mime="text/csv")
...
if evento.selection and evento.selection.rows:
    ...
    st.markdown("---")
    st.markdown(f'<h3 class="section-header">📈 Análise Técnica: {ticker} — {row["Empresa"]}</h3>',
                unsafe_allow_html=True)
    ...
    with col_graf:
        try:
            df_t = df_calc.xs(ticker, axis=1, level=1).dropna()
            fig  = plotar_grafico(df_t, ticker)
    ...
    with col_info:
        ...
        st.markdown(f"""
        <div style="background:{cor_bg};padding:0.8rem;border-radius:8px;margin-bottom:0.75rem;text-align:center;">
        ...
        st.metric("📊 RSI14",       f"{row['RSI14']:.1f}")
        st.metric("📊 Stoch %K",    f"{row['Stoch']:.1f}")
        if row['Gap'] < -1:
            st.metric("⚡ Gap Abertura", f"{row['Gap']:.2f}%", delta_color="inverse")
        st.metric("💧 Liquidez",    f"{row['Liquidez']}/10")
        st.metric("📋 Score Técnico", f"{row['Score']}/10")

        st.markdown("""
        <div style="background:#e0e7ff;padding:0.6rem;border-radius:6px;margin-top:0.75rem;">
            <b style="color:#3730a3;font-size:0.85rem;">📋 Sinais Detectados</b>
        </div>""", unsafe_allow_html=True)
        ...
        if row.get('Explicacoes'):
            ...
            <div style="background:#fef3c7;padding:0.6rem;border-radius:6px;margin-top:0.5rem;">
                <b style="color:#92400e;font-size:0.85rem;">💡 O que isso significa?</b>
            </div>""", unsafe_allow_html=True)
    ...
    if fd:
        score = fd['score']
        st.markdown(f"""
        <div style="background:{fd['grad']};padding:1.5rem;border-radius:12px;margin-bottom:1.5rem;text-align:center;">
            <h1 style="margin:0;color:{fd['tc']};font-size:4rem;font-weight:900;">{score:.0f}/100</h1>
            <p style="margin:0.4rem 0 0 0;color:{fd['tc']};font-size:1.4rem;font-weight:600;">{fd['label']}</p>
        </div>""", unsafe_allow_html=True)
        ...
        with col_v:
            ...
            st.metric("P/E Ratio",   f"{det['P/E']:.2f}"    if det.get('P/E') else "N/A")
            mc = det.get('MarketCap')
            st.metric("Market Cap",  (f"${mc/1e12:.2f}T" if mc >= 1e12 else f"${mc/1e9:.1f}B") if mc else "N/A")

        with col_r:
            st.markdown("### 💰 Rentabilidade")
            st.metric("Dividend Yield",      f"{det['DY']:.2f}%"          if det.get('DY') else "N/A")
            rg_v = det.get('RevGrowth')
            if rg_v is not None:
        ...
        with col_i:
            ...
            st.markdown(f"<h3 style='color:{cor_rec};margin-top:0.5rem;'>{rec_str}</h3>",
                        unsafe_allow_html=True)
            if fd.get('setor'):
        ...
        linhas = []
        if det.get('P/E'):
            linhas.append({'Métrica': 'P/E Ratio', 'Valor': f"{det['P/E']:.2f}",
                           'Pontos': det.get('P/E_pts','+0'), 'Avaliação': det.get('P/E_av','')})
        if det.get('DY'):
            linhas.append({'Métrica': 'Dividend Yield', 'Valor': f"{det['DY']:.2f}%",
                           'Pontos': det.get('DY_pts','+0'), 'Avaliação': det.get('DY_av','')})
        if det.get('RevGrowth') is not None:
            linhas.append({'Métrica': 'Crescimento Receita', 'Valor': f"{det['RevGrowth']:+.1f}%",
        ...
        if det.get('Rec'):
            ...
            linhas.append({'Métrica': 'Recomendação Analistas', 'Valor': det['Rec'],
                           'Pontos': pts_r.get(fd.get('rec_raw',''),'+0'), 'Avaliação': det.get('Rec','')})
        mc = det.get('MarketCap')
        if mc:
        ...
        if linhas:
            ...
            st.caption(f"Score Total: {score}/100 (Base 50 + bônus/penalidades)")
    else:
        st.warning(f"⚠️ Dados fundamentalistas indisponíveis para {ticker}")
        st.info("""
        **Possíveis causas:**
        - ETF (dados fundamentalistas não aplicáveis)
        - Ativo com baixíssimo volume ou recém-listado
        - Timeout na API do Yahoo Finance — tente novamente
        """)

else:
    st.markdown("""
    <div class="select-prompt">
        <p>👆 Selecione uma ação na tabela acima para visualizar o gráfico técnico completo e a análise fundamentalista</p>
    </div>""", unsafe_allow_html=True)
